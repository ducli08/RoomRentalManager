...
export class RoomrentalsComponent {
  ...
  imageDescriptionSrc(pathOrUrl?: string | null): string {
    return resolveAssetUrl(this.baseUrl, pathOrUrl ?? undefined);
  }
  ...
  openEditRoomRentalModal(roomrental: RoomRentalDto): void {
    const modalRef = this.modalService.create<EditRoomRentalsComponent, { roomrentalData: any }, string>({
    ...
    modalRef.afterOpen.pipe(take(1)).subscribe(() => {
      const edit = modalRef.getContentComponent() as EditRoomRentalsComponent | undefined;
