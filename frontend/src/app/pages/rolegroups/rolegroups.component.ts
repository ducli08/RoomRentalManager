...
export class RoleGroupsComponent {
  ...
  openEditRoleGroupModal(roleGroup: RoleGroupDto): void {
    const modal = this.modalService.create<EditRoleGroupsComponent, { roleGroupData: RoleGroupDto }, string>({
    ...
    const contentComp = modal.getContentComponent() as EditRoleGroupsComponent | null;
    if (contentComp) {
      const sub = contentComp.saved.pipe(take(1)).subscribe(() => {
        this.getAllRoleGroups();
        this.notification.success('Thành công', 'Nhóm quyền đã được cập nhật.');
        modal.close();
        sub.unsubscribe();
      });
    }
