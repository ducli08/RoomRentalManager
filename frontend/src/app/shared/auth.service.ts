...
export class AuthService {
    ...
    return this.proxy.login(username, password, rememberMe).pipe(
      tap(res => {
        if (res && res.accessToken) {
          this.completeLogin(res, rememberMe);
        }
      })
    );
  }
  ...
  /**
   * Lưu token và đồng bộ `currentUser$` sau mọi đăng nhập thành công (form, Google, …).
   */
  completeLogin(res: LoginResponseDto, remember: boolean): void {
    if (!res?.accessToken) {
      return;
    }
    this.saveTokens(res, remember);
    const u = res.user;
    if (u && (u.name || u.avatar)) {
      const cur = this.currentUser$.value;
      const username = u.name && String(u.name).trim() ? u.name : cur?.username;
      let avatarUrl = cur?.avatarUrl;
      if (u.avatar != null && u.avatar !== '') {
        avatarUrl = u.avatar;
      }
      this.currentUser$.next({
        username: username ?? cur?.username ?? 'User',
        avatarUrl,
        userId: cur?.userId
      });
    }
  ...
  logout(): void {
    ...
    if (refreshToken) {
      const body = new LogoutDto({ refreshToken });
      this.proxy.logout(body).pipe(
        catchError(() => of(null)),
        finalize(() => finish())
      ).subscribe();
    } else {
  ...
  private setCurrentUserFromToken() {
    ...
    const username = payload['http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'] as string | undefined;
  ...
  private saveTokens(res: LoginResponseDto, remember: boolean) {
    ...
    if (res.user?.avatar != null && res.user?.avatar !== '') {
      if (remember) {
        ...
        sessionStorage.setItem('avatar_url', res.user.avatar);
